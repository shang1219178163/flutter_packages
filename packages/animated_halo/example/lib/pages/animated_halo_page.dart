...
import '../widgets/app_res.dart';
import '../widgets/n_choice_color_list_item.dart';
import '../widgets/n_decoration_card.dart';
import '../widgets/n_description_card.dart';
import '../widgets/n_network_image.dart';
import '../widgets/n_slider_list_item.dart';

class AnimatedHaloPage extends StatefulWidget {
  const AnimatedHaloPage({super.key, this.title});
...
class _AnimatedHaloPageState extends State<AnimatedHaloPage> {
  ...
  /// 何时隐藏 AppBar：当页面通过命名路由展示且路由名命中 widget 名时。
  bool _hideAppBar(BuildContext context) {
    final routeName = ModalRoute.of(context)?.settings.name ?? '';
    if (routeName.isEmpty) {
      return false;
    }
    return '$widget'.toLowerCase().endsWith(routeName.toLowerCase());
  }

  @override
  Widget build(BuildContext context) {
    final scheme = theme.colorScheme;
    final hideApp = _hideAppBar(context);
    return Scaffold(
